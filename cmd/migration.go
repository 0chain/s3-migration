...
import (
	...
	"github.com/0chain/s3-migration/util"
...
func GetMigrationConfig(cmd *cobra.Command) (*MigrationConfig, error) {
	...
	if fflags.Changed("bucket") { // set bucket
	...
	if fflags.Changed("region") { // check if the flag "region" is set
	...
	if !fflags.Changed("allocation") { // check if the flag "allocation" is set
...
func startS3Upload(allocationObj *sdk.Allocation, fileReader io.Reader, size int64, mimeType, remotePath string, encrypt bool, attrs fileref.Attributes, statusBar sdk.StatusCallback) error {
	...
	ChunkedUpload, err := sdk.CreateChunkedUpload(util.GetDefaultConfigDir(), allocationObj, fileMeta, fileReader, false,
